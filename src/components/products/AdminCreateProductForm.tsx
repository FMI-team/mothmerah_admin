...
import { createProductForSeller } from "../../../services/products";
...
export default function AdminCreateProductForm({
  ...
  const fetchCurrentUser = useCallback(async () => {
    try {
      ...
      const typeKey =
        user.user_type?.user_type_name_key ??
        user.default_role?.role_name_key ??
        null;
  ...
  const handleSubmit = async (e: SubmitEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (countryOfOriginCode.trim()) {
        requestBody.country_of_origin_code = countryOfOriginCode.trim();
      }
      if (mainImageUrl.trim()) {
        requestBody.main_image_url = mainImageUrl.trim();
      }
      if (sku.trim()) {
        requestBody.sku = sku.trim();
      }

      const response = await createProductForSeller(requestBody);
                    ...
                    <div className="relative">
                      <Select
                        options={wholesalerUsers.map((u) => ({
                          value: u.user_id,
                          label: `${u.first_name} ${u.last_name} (${u.phone_number})`,
                        }))}
                        placeholder={
