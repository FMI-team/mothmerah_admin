...
export default function CreateProductForm({
  ...
  const handleSubmit = async (e: SubmitEvent<HTMLFormElement>) => {
    ...
    const hasValidTranslations = translationsData.some((t) => t.translated_product_name && t.translated_description && t.translated_short_description);
    if (!hasValidTranslations) {
      setError("يرجى إدخال الترجمات (اسم المنتج، الوصف، والوصف المختصر)");
      return;
    }

    const hasValidPackaging = packagingOptionsData.some((p) => p.quantity_in_packaging.trim() !== "" && p.unit_of_measure_id_for_quantity && p.base_price.trim() !== "");
    ...
    try {
      ...
      const validatedPackagingOptions = packagingOptionsData.map((pkg) => ({
        ...
        unit_of_measure_id_for_quantity: parseInt(pkg.unit_of_measure_id_for_quantity, 10) || 0,
        ...
        translations: pkg.translations.filter(
          (tr) => tr.translated_packaging_option_name || tr.translated_custom_description).map((tr) => ({
      ...
      form.append("category_id", String(parseInt(categoryId, 10)));
      form.append("base_price_per_unit", String(parseFloat(basePricePerUnit) || 0));
      form.append("unit_of_measure_id", String(parseInt(unitOfMeasureId, 10) || 0));
      form.append("translations", JSON.stringify(validatedTranslations));
      form.append("packaging_options", JSON.stringify(validatedPackagingOptions));
      form.append("is_organic", isOrganic ? "true" : "false");
      form.append("is_local_saudi_product", isLocalSaudiProduct ? "true" : "false");
      if (countryOfOriginCode.trim()) form.append("country_of_origin_code", countryOfOriginCode.trim());
      if (mainImageUrl.trim()) form.append("main_image_url", mainImageUrl.trim());
  ...
  const categoryOptions = categories.filter((cat) => cat.is_active).map((cat) => {
    ...
    <Modal isOpen={isOpen} onClose={onClose} className="max-w-[800px] m-4">
      <div className="no-scrollbar relative w-full max-w-[800px] overflow-y-auto rounded-3xl bg-white p-4 dark:bg-gray-900 lg:p-11">
        <div className="px-2 pr-14">
          <h4 className="mb-2 text-2xl font-semibold text-gray-800 dark:text-white/90">إنشاء منتج جديد</h4>
          <p className="mb-6 text-sm text-gray-500 dark:text-gray-400 lg:mb-7">املأ المعلومات التالية لإنشاء منتج جديد</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="flex flex-col">
          <div className="custom-scrollbar h-[800px] overflow-y-auto px-2 pb-3">
            <div className="space-y-6">
              <div>
                <Label>الفئة <span className="text-error-500">*</span>
                </Label>
                <div className="relative">
                  <Select options={categoryOptions} placeholder={isLoadingCategories ? "جاري التحميل..." : "اختر الفئة"} onChange={(value) => setCategoryId(value)} defaultValue={categoryId}
              ...
              <div>
                ...
                <Input type="number" step={0.01} placeholder="0.00" defaultValue={basePricePerUnit} onChange={(e) => setBasePricePerUnit(e.target.value)} />
              </div>

              <div>
                <Label>وحدة القياس</Label>
                <div className="relative">
                  <Select options={UNITS_OF_MEASURE.map((unit: UnitOfMeasure) => ({
                      value: unit.unit_id.toString(),
                      label: `${unit.unit_name_key} (${unit.unit_abbreviation_key})`,
                    }))} placeholder="اختر وحدة القياس" onChange={(value) => setUnitOfMeasureId(value)} defaultValue={unitOfMeasureId} className="dark:bg-dark-900" />
                </div>
              </div>

              <div>
                <Label>رمز بلد المنشأ</Label>
                <input type="text" value={countryOfOriginCode} onChange={(e) => setCountryOfOriginCode(e.target.value)} placeholder="مثال: SA" className="h-11 w-full rounded-lg border
                border-gray-300 bg-white px-4 py-2.5 text-sm text-gray-800 shadow-theme-xs placeholder:text-gray-400 focus:border-brand-300 focus:outline-none focus:ring-3
                focus:ring-brand-500/10 dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30 dark:focus:border-brand-800" />
              </div>
              ...
              <div>
                ...
                <input type="url" value={mainImageUrl} onChange={(e) => setMainImageUrl(e.target.value)} placeholder="https://example.com/image.jpg" className="h-11 w-full rounded-lg border
                border-gray-300 bg-white px-4 py-2.5 text-sm text-gray-800 shadow-theme-xs placeholder:text-gray-400 focus:border-brand-300 focus:outline-none focus:ring-3
                focus:ring-brand-500/10 dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30 dark:focus:border-brand-800" />
              </div>
              <div>
                <Label>رمز المنتج (sku)</Label>
                <input type="text" value={sku} onChange={(e) => setSku(e.target.value)} placeholder="SKU-12345" className="h-11 w-full rounded-lg border border-gray-300 bg-white px-4 py-2.5
              ...
              <div className="space-y-4">
                <Label> الترجمات <span className="text-error-500">*</span> </Label>
                {translationsData.map((translation, index) => (
                  <div key={index} className="rounded-lg border border-gray-200 p-4 dark:border-gray-700">
                    <div className="mb-3 flex items-center justify-between">
                      <h5 className="text-sm font-medium text-gray-700 dark:text-gray-300"> {translation.language_code === "ar" ? "العربية" : "English"} </h5>
                      <span className="text-xs text-gray-500 dark:text-gray-400"> {translation.language_code.toUpperCase()} </span>
                    </div>
                    <div className="space-y-3">
                      <div>
                        <Label className="text-xs">اسم المنتج *</Label>
                        <input type="text" placeholder={translation.language_code === "ar" ? "طماطم عضوية طازجة" : "Fresh Organic Tomatoes"} value={translation.translated_product_name}
                        onChange={(e) => {
                          const updated = [...translationsData];
                          updated[index].translated_product_name = e.target.value;
                          setTranslationsData(updated);
                        }} className="h-11 w-full rounded-lg border border-gray-300 bg-white px-4 py-2.5 text-sm shadow-theme-xs placeholder:text-gray-400 focus:border-brand-300
                      ...
                      <div>
                        ...
                        <textarea className="h-20 w-full rounded-lg border border-gray-300 bg-white px-4 py-2.5 text-sm shadow-theme-xs placeholder:text-gray-400 focus:border-brand-300 
                        ocus:outline-hidden focus:ring-3 focus:ring-brand-500/10 dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30
                        dark:focus:border-brand-800" placeholder={translation.language_code === "ar" ? "طماطم عضوية طازجة من المزارع المحلية السعودية، خالية من المبيدات" : "Fresh organic tomatoes from local Saudi farms, pesticide-free"}
                        value={translation.translated_description} onChange={(e) => {
                          const updated = [...translationsData];
                      ...
                      <div>
                        ...
                        <input type="text" placeholder={translation.language_code === "ar" ? "طماطم عضوية طازجة" : "Fresh organic tomatoes"} value={translation.translated_short_description}
                        onChange={(e) => {
                          const updated = [...translationsData];
              ...
              <div className="space-y-4">
                <div className="flex items-center justify-between">
                  <Label> خيارات التعبئة <span className="text-error-500">*</span> </Label>
                  <button type="button" onClick={() => {
                    setPackagingOptionsData([...packagingOptionsData, {
                      quantity_in_packaging: "",
                      unit_of_measure_id_for_quantity: "",
                      base_price: "",
                      sku: "",
                ...
                }} className="inline-flex items-center justify-center gap-2 rounded-lg border border-gray-300 bg-white px-3 py-1.5 text-xs font-medium text-gray-700 shadow-theme-xs
                hover:bg-gray-50 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-300 dark:hover:bg-gray-700"> + إضافة خيار تعبئة
                  </button>
                </div>
                  ...
                  <div key={pkgIndex} className="rounded-lg border border-gray-200 p-4 dark:border-gray-700">
                    <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
                      <div>
                        ...
                        <input type="number" step="0.01" min="0" placeholder="1" value={pkg.quantity_in_packaging} onChange={(e) => {
                      ...
                      <div>
                        ...
                        <Select options={UNITS_OF_MEASURE.map((unit) => ({
                          value: unit.unit_id.toString(),
                          label: `${unit.unit_name_key} (${unit.unit_abbreviation_key})`
                        }))} placeholder="اختر وحدة القياس" onChange={(value) => {
                          const updated = [...packagingOptionsData];
                          updated[pkgIndex].unit_of_measure_id_for_quantity = value;
                          setPackagingOptionsData(updated);
                        }} defaultValue={pkg.unit_of_measure_id_for_quantity} className="dark:bg-dark-900" />
                      </div>
                      <div>
                        <Label className="text-xs">السعر الأساسي *</Label>
                        <input type="number" step="0.01" placeholder="25.5" value={pkg.base_price} onChange={(e) => {
                          const updated = [...packagingOptionsData];
                      ...
                      <div>
                        ...
                        <input type="number" placeholder="1" value={pkg.sort_order} onChange={(e) => {
                          const updated = [...packagingOptionsData];
                      ...
                      <div>
                        <Label className="text-xs">SKU <span className="text-error-500">*</span></Label>
                        <input type="text" placeholder="PKG-001-1KG" value={pkg.sku} onChange={(e) => {
                          const updated = [...packagingOptionsData];
                      ...
                      <div>
                        <Label className="text-xs">الباركود <span className="text-error-500">*</span></Label>
                        <input type="text" placeholder="1234567890123" value={pkg.barcode} onChange={(e) => {
                          const updated = [...packagingOptionsData];
                    ...
                    <div className="mt-4 space-y-3">
                      ...
                      {pkg.translations.map((translation, transIndex) => (
                        <div key={transIndex} className="rounded border border-gray-200 p-3 dark:border-gray-700">
                          <div className="mb-2 text-xs font-medium text-gray-600 dark:text-gray-400"> {translation.language_code === "ar" ? "العربية" : "English"} </div>
                          <div className="space-y-2">
                            <div>
                              <Label className="text-xs">اسم خيار التعبئة *</Label>
                              <input required type="text" placeholder={translation.language_code === "ar" ? "كيلوغرام واحد" : "One Kilogram"} value={translation.translated_packaging_option_name}
                              onChange={(e) => {
                                const updated = [...packagingOptionsData];
                            ...
                            <div>
                              ...
                              <input type="text" placeholder={translation.language_code === "ar" ? "عبوة واحدة كيلوغرام" : "Single 1kg package"} value={translation.translated_custom_description}
                              onChange={(e) => {
                                const updated = [...packagingOptionsData];
          ...
          <div className="flex items-center gap-3 px-2 mt-6 lg:justify-end">
            <Button size="sm" variant="outline" onClick={onClose} disabled={isSubmitting}>إلغاء</Button>
            <button type="submit" disabled={isSubmitting} className="inline-flex items-center justify-center gap-2 rounded-lg px-4 py-3 text-sm font-medium transition bg-brand-500 text-white
            shadow-theme-xs hover:bg-brand-600 disabled:bg-brand-300 disabled:cursor-not-allowed disabled:opacity-50">
