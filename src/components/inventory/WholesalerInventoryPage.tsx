...
export default function WholesalerInventoryPage() {
  ...
  const fetchInventory = useCallback(async () => {
    ...
    setError(null);
    try {
      const response = await readMyInventory()
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-gray-800 dark:text-white/90">ادارة المخزون</h1>
        <p className="mt-2 text-gray-500 dark:text-gray-400">عرض وإدارة أصناف المخزون والكميات المتاحة</p>
      </div>
            ...
            {totalPages > 1 && (
              <div className="flex items-center justify-between gap-4 pt-6">
                ...
                <div className="flex items-center gap-2">
                  <button onClick={() => setCurrentPage((p) => Math.max(1, p - 1))} disabled={currentPage === 1} className="rounded-lg border border-gray-200 bg-white px-4 py-2 text-sm
                  ...
                  <button onClick={() => setCurrentPage((p) => Math.min(totalPages, p + 1))} disabled={currentPage >= totalPages} className="rounded-lg border border-gray-200 bg-white px-4
      ...
      <Modal isOpen={!!adjustStockItem} onClose={closeAdjustModal} className="max-w-[420px] p-5 lg:p-6">
        <form onSubmit={handleAdjustStock}>
          ...
          <div className="mb-4">
            ...
            <input type="number" value={adjustQuantity} onChange={(e) => setAdjustQuantity(e.target.value)} placeholder="مثال: 5 للإضافة، -3 للخصم" step={1} autoFocus className="h-11 w-full
            rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-800 shadow-theme-xs placeholder:text-gray-400 focus:border-brand-300 focus:outline-none
            ...
            />
            <p className="mt-1.5 text-xs text-gray-500 dark:text-gray-400">استخدم قيمة موجبة للإضافة وسالبة للخصم</p>
          </div>
          {adjustError && (
          ...
          <div className="flex items-center justify-end gap-3">
            <button type="button" onClick={closeAdjustModal} className="inline-flex items-center justify-center gap-2 rounded-lg bg-white px-4 py-3 text-sm font-medium text-gray-700 ring-1
            ring-inset ring-gray-300 transition hover:bg-gray-50 dark:bg-gray-800 dark:text-gray-400 dark:ring-gray-700 dark:hover:bg-white/3 dark:hover:text-gray-300">
              إلغاء
            </button>
            <button type="submit" disabled={isAdjusting} className="inline-flex items-center justify-center gap-2 rounded-lg bg-brand-500 px-4 py-3 text-sm font-medium text-white
