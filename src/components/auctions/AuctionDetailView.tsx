...
import { deleteAuctionById, readAuctionById } from "../../../services/auctions";
...
export default function AuctionDetailView() {
    ...
    const fetchAuctionDetails = async () => {
      ...
      try {
        ...
        if (status === 404) {
          ...
          setError("ليس لديك صلاحية لعرض هذا المزاد");
        } else {
          setError(
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      console.log(err);
      setError(err instanceof Error ? err.message : "حدث خطأ في حذف المزاد");
    } finally {
      setIsDeleting(false);
    }
  };
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-2">
          <a href={editHref} className="inline-flex items-center justify-center gap-2 rounded-lg bg-brand-500 px-4 py-2.5 text-sm font-medium text-white shadow-theme-xs transition
          hover:bg-brand-600">تعديل</a>
          <button type="button" onClick={handleDelete} disabled={isDeleting} className="inline-flex items-center justify-center gap-2 rounded-lg border border-error-300 bg-error-50 px-4 py-2.5
          text-sm font-medium text-error-700 transition hover:bg-error-100 dark:border-error-800 dark:bg-error-900/20 dark:text-error-400 dark:hover:bg-error-900/30
          disabled:opacity-50"> {isDeleting ? "جاري الحذف..." : "حذف"} </button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
        <div className="lg:col-span-2 space-y-6">
          <div className="p-5 border border-gray-200 rounded-2xl dark:border-gray-800 lg:p-6">
            <h3 className="mb-4 text-lg font-semibold text-gray-800 dark:text-white/90">معلومات المزاد</h3>
            <div className="space-y-4">
              <div>
                <Label>عنوان المزاد</Label>
                <p className="mt-1 text-sm font-medium text-gray-800 dark:text-white/90"> {auction.custom_auction_title || "بدون عنوان"} </p>
              </div>
              {auction.custom_auction_description && (
                <div>
                  <Label>وصف المزاد</Label>
                  <p className="mt-1 text-sm text-gray-600 dark:text-gray-400"> {auction.custom_auction_description} </p>
                </div>
              )}
              <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                <div>
                  <Label>نوع المزاد</Label>
                  <p className="mt-1 text-sm font-medium text-gray-800 dark:text-white/90"> {typeName} </p>
                </div>
                <div>
                ...
                <div>
                  ...
                  <p className="mt-1 text-sm font-medium text-gray-800 dark:text-white/90"> {auction.is_private_auction ? "نعم" : "لا"} </p>
                </div>
                <div>
                  <Label>عدد المزايدات</Label>
                  <p className="mt-1 text-sm font-medium text-gray-800 dark:text-white/90"> {auction.total_bids_count} </p>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="p-5 border border-gray-200 rounded-2xl dark:border-gray-800 lg:p-6">
            <h3 className="mb-4 text-lg font-semibold text-gray-800 dark:text-white/90">معلومات المنتج</h3>
            <div className="space-y-4">
              ...
              <div>
                ...
                <p className="mt-1 text-sm font-medium text-gray-800 dark:text-white/90"> {productName} </p>
              </div>
              {productDescription && (
                <div>
                  <Label>وصف المنتج</Label>
                  <p className="mt-1 text-sm text-gray-600 dark:text-gray-400"> {productDescription} </p>
                </div>
              )}
              <div>
                <Label>الفئة</Label>
                <p className="mt-1 text-sm font-medium text-gray-800 dark:text-white/90"> {categoryName} </p>
              </div>
            </div>
          </div>
          ...
          {auction.lots && auction.lots.length > 0 && (
            <div className="p-5 border border-gray-200 rounded-2xl dark:border-gray-800 lg:p-6">
              <h3 className="mb-4 text-lg font-semibold text-gray-800 dark:text-white/90">اللوتات ({auction.lots.length})</h3>
              <div className="space-y-4">
                {auction.lots.map((lot, index) => (
                  <div key={lot.lot_id} className="p-4 border border-gray-200 rounded-xl dark:border-gray-800">
                    <div className="flex items-start justify-between mb-2">
                      <h4 className="font-medium text-gray-800 dark:text-white/90"> {lot.custom_lot_title || `لوت ${index + 1}`} </h4>
                    </div>
                    {lot.custom_lot_description && (
                      <p className="mb-3 text-sm text-gray-600 dark:text-gray-400"> {lot.custom_lot_description} </p>
                    )}
                    <div className="grid grid-cols-2 gap-4 text-sm">
                      <div>
                        <span className="text-gray-500 dark:text-gray-400">الكمية: </span>
                        <span className="font-medium text-gray-800 dark:text-white/90"> {lot.quantity_in_lot} </span>
                      </div>
                      <div>
                        <span className="text-gray-500 dark:text-gray-400">السعر الابتدائي: </span>
                        <span className="font-medium text-gray-800 dark:text-white/90"> {lot.lot_starting_price.toFixed(2)} ر.س </span>
                      </div>
                    </div>
                  </div>
                ))}
        ...
        <div className="space-y-6">
          <div className="p-5 border border-gray-200 rounded-2xl dark:border-gray-800 lg:p-6">
            <h3 className="mb-4 text-lg font-semibold text-gray-800 dark:text-white/90">معلومات التسعير</h3>
            <div className="space-y-4">
              <div>
                <Label>السعر الابتدائي لكل وحدة</Label>
                <p className="mt-1 text-lg font-bold text-gray-800 dark:text-white/90"> {auction.starting_price_per_unit.toFixed(2)} ر.س </p>
              </div>
              <div>
                <Label>أعلى مزايدة حالية</Label>
                <p className="mt-1 text-lg font-bold text-purple-600 dark:text-purple-400"> {(auction.current_highest_bid_amount_per_unit || auction.starting_price_per_unit).toFixed(2)} ر.س </p>
              </div>
              <div>
                <Label>الحد الأدنى للزيادة</Label>
                <p className="mt-1 text-sm font-medium text-gray-800 dark:text-white/90"> {auction.minimum_bid_increment.toFixed(2)} ر.س </p>
              </div>
              {auction.reserve_price_per_unit && (
                <div>
                  <Label>السعر الاحتياطي</Label>
                  <p className="mt-1 text-sm font-medium text-gray-800 dark:text-white/90"> {auction.reserve_price_per_unit.toFixed(2)} ر.س </p>
                </div>
              )}
              <div>
                <Label>الكمية المعروضة</Label>
                <p className="mt-1 text-sm font-medium text-gray-800 dark:text-white/90"> {auction.quantity_offered} </p>
              </div>
            </div>
          </div>

          <div className="p-5 border border-gray-200 rounded-2xl dark:border-gray-800 lg:p-6">
            <h3 className="mb-4 text-lg font-semibold text-gray-800 dark:text-white/90">معلومات البائع</h3>
            <div className="space-y-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="font-medium text-gray-800 dark:text-white/90"> {`${auction.seller.first_name} ${auction.seller.last_name}`} </p>
                  <p className="text-sm text-gray-500 dark:text-gray-400"> {auction.seller.email} </p>
                </div>
              </div>
              <div>
                <Label>رقم الهاتف</Label>
                <p className="mt-1 text-sm font-medium text-gray-800 dark:text-white/90"> {auction.seller.phone_number} </p>
              </div>
            </div>
          </div>

          <div className="p-5 border border-gray-200 rounded-2xl dark:border-gray-800 lg:p-6">
            <h3 className="mb-4 text-lg font-semibold text-gray-800 dark:text-white/90">التواريخ</h3>
            <div className="space-y-4">
              <div>
                <Label>تاريخ البدء</Label>
                <p className="mt-1 text-sm font-medium text-gray-800 dark:text-white/90"> {formatDate(auction.start_timestamp)} </p>
              </div>
              <div>
                <Label>تاريخ الانتهاء</Label>
                <p className="mt-1 text-sm font-medium text-gray-800 dark:text-white/90"> {formatDate(auction.end_timestamp)} </p>
              </div>
            </div>
          </div>
        </div>
