      ...
      <div className="p-5 border border-gray-200 rounded-2xl dark:border-gray-800 lg:p-6">
        <div className="flex flex-col gap-6 lg:flex-row lg:items-start lg:justify-between">
          ...
          >
            ...
            {t("users.profile.addressCard.edit")}
          </button>
        </div>
      </div>
        ...
        <div className="relative w-full p-4 overflow-y-auto bg-white no-scrollbar rounded-3xl dark:bg-gray-900 lg:p-11">
          <div className="px-2 pr-14">
            <h4 className="mb-2 text-2xl font-semibold text-gray-800 dark:text-white/90">
              {t("users.profile.addressCard.editModal.title")}
            </h4>
            <p className="mb-6 text-sm text-gray-500 dark:text-gray-400 lg:mb-7">
              {t("users.profile.addressCard.editModal.description")}
            </p>
          </div>
          <form className="flex flex-col">
            <div className="px-2 overflow-y-auto custom-scrollbar">
              <div className="grid grid-cols-1 gap-x-6 gap-y-5 lg:grid-cols-2">
                <div>
                  <Label>{t("users.profile.addressCard.editModal.country")}</Label>
                  <Input type="text" defaultValue="United States" />
                </div>

                <div>
                  <Label>{t("users.profile.addressCard.editModal.cityState")}</Label>
                  <Input type="text" defaultValue="Arizona, United States." />
                </div>

                <div>
                  <Label>{t("users.profile.addressCard.editModal.postalCode")}</Label>
                  <Input type="text" defaultValue="ERT 2489" />
                </div>

                <div>
                  <Label>{t("users.profile.addressCard.editModal.taxId")}</Label>
            ...
            <div className="flex items-center gap-3 px-2 mt-6 lg:justify-end">
              <Button size="sm" variant="outline" onClick={closeModal}>
                {t("users.profile.addressCard.editModal.close")}
              </Button>
              <Button size="sm" onClick={handleSave}>
                {t("users.profile.addressCard.editModal.saveChanges")}
              </Button>
            </div>
          </form>
        </div>
